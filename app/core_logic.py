...
# 🔹 Load LLM (Mac Optimized)
def load_model(device="cpu"):
    # Adjust model path for Mac
    model_path = os.path.join("models", "mistral-7b-instruct-v0.1.Q4_K_M.gguf")

    # Decide GPU layers
    gpu_layers = 40 if device != "cpu" else 0
...
# 🔹 Load All Together
def load_model_and_index(doc_folder="docs", device="cpu"):
...
# 🔹 Chat Function (Core)
def chat_with_ecofriend(user_input, eco_level, model, embedder, index, chunks, device="cpu"):
